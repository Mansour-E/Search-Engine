...
// Exercise 2
    ...
    public void createCombinedScoreView() {
        try (Statement stmt = connection.createStatement()) {
            String createCombinedViewSQL = """
        CREATE OR REPLACE VIEW combined_score_view AS
        SELECT
            bm25.docid,
            bm25.term,
            bm25.bm25_score,
            pr.page_rank,
            (0.5 * bm25.bm25_score + 0.5 * pr.page_rank) AS combined_score
        FROM bm25_view bm25
        JOIN page_rank_view pr ON bm25.docid = pr.docid;
        """;
            stmt.executeUpdate(createCombinedViewSQL);
            System.out.println("Combined Score View created");
        } catch (SQLException e) {
            System.err.println("Fehler beim Erstellen der Combined Score View: " + e.getMessage());
            e.printStackTrace();
        }
    }
    ...
    public void createViews() {
        ...
        """;
            ...
            // Combined Score View erstellen
            createCombinedScoreView();
        } catch (SQLException e) {
